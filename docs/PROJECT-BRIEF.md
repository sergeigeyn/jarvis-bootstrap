...
Централизованная панель для оператора (владельца проекта). Цель: видеть всех развёрнутых агентов, управлять ими, пушить обновления.

**Минимальный функционал:**
- [ ] Список всех развёрнутых агентов (сервер, IP, статус, версия, движок)
- [ ] Статус каждого: online/offline, uptime, последняя активность
- [ ] Push-обновления: git pull + restart на всех агентах одной кнопкой
- [ ] Логи агента (journalctl)
- [ ] Версия кода (git rev) и расхождение с main

**Расширенный функционал:**
- [ ] Метрики: сколько сообщений обработано, расходы, ошибки
- [ ] Здоровье: диск, память, CPU
- [ ] Управление: перезапуск, остановка, удаление агента
- [ ] Конфигурация: смена движка, обновление ключей
- [ ] Уведомления: агент упал, диск заканчивается, ошибки

**Архитектура (предложение):**
```
Dashboard (web)  ←→  API  ←→  Агенты на VPS
                              (health-check endpoint
                               или SSH/API polling)
```

Агенты должны уметь отчитываться о своём состоянии. Варианты:
1. HTTP endpoint на каждом агенте (минимальный health-check сервер)
2. Агенты пингуют центральный сервер (phone-home)
3. SSH polling с dashboard'а

### Installer bot (helper-aishnik)
- [ ] Выложить на GitHub
- [ ] Сделать systemd-сервис (сейчас запускается вручную)
- [ ] Auto-update развёрнутых агентов (git pull + restart через SSH)
