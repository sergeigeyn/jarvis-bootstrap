...
После ответа — **футер с таймером** (🕐 35.0s) в составе последнего сообщения. Стоимость ($0.042) показывается только при оплате через API, при подписке — только время. Если агент использовал инструменты (Read, Edit, Bash и т.д.) — добавляются **инлайн-кнопки** (✔ Продолжай / ✖ Стоп / 💬 Комментарий).
