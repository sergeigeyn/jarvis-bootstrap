...
После ответа — **футер** как `<blockquote>⏱ 35s</blockquote>` в составе последнего сообщения. Стоимость ($0.042) показывается только при оплате через API, при подписке — только время. Если агент использовал инструменты (Read, Edit, Bash и т.д.) — добавляются **инлайн-кнопки** (✔ Продолжай / ✖ Стоп / 💬 Комментарий).
