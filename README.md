...
После ответа — **футер с таймером** (🕐 35.0s) в составе последнего сообщения. Стоимость ($0.042) показывается только при оплате через API, при подписке — только время. Для задач дольше 5 секунд добавляются **инлайн-кнопки** (✔ Продолжай / ✖ Стоп / 💬 Комментарий).
