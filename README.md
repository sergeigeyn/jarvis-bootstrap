...
Бот принимает текст/голос/фото/файлы → вызывает CLI-агент → **парсит stream-json в реальном времени** (прогресс-статусы) → **hooks проверяют ответ** → парсит медиа-маркеры → отправляет в Telegram.

Во время работы бот показывает live-статус: «🤔 Мозгую... 15с», «Читаю файл 🌚 23с», «Редактирую 🌚 45с». Если отправить новый запрос пока бот занят — покажет «⏳ в очереди» и обработает после.
