...
После ответа — **футер** как `<blockquote>⏱ 35s · $0.042</blockquote>` в составе последнего сообщения. Стоимость показывается всегда (если доступна). Если агент упёрся в лимит turns (20+ из 25) — добавляются **инлайн-кнопки** (✔ Продолжай / ✖ Стоп / 💬 Комментарий).
